				...
				for (var col = 0; col < nCount; col++) {
					if((row>=nCountIndexMin && row<nCountIndexMax)){
						if(col>=nCountIndexMin && col<nCountIndexMax){
							...
							aHTML.push(drawCell(nWidth, nHeight,(oQRCode.isDark(row, col) ? _htOption.colorDark : _htOption.colorLight)));
						}
					}else{
						aHTML.push(drawCell(nWidth, nHeight,(oQRCode.isDark(row, col) ? _htOption.colorDark : _htOption.colorLight)));
		...
		Drawing.prototype.draw = async function (oQRCode) {
			...
			for (var row = 0; row < nCount; row++) {
				for (var col = 0; col < nCount; col++) {
					...
					if((row>(nCount/2)-4 && row<(nCount/2)+4)&&
						(col>(nCount/2)-4 && col<(nCount/2)+4)){
			...
			let x =((nCount/2)-3)*nWidth;
			let y = ((nCount/2)-3)*nHeight;
			await waitImage(_oContext,img, x , y, nWidth, nHeight);
